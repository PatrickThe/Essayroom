...
<head>
  ...
  <link rel="stylesheet" href="../Order/css/css-fontawesome-all.min.css"><!-- Custom styles for this template -->
  <link href="../Order/css/css-bootstrap.min.css" rel="stylesheet">
  <script src="../Order/js/js-jquery.min.js"></script>
  <script src="../Order/js/js-bootstrap.min.js"></script>
</head>
...
<script type="text/javascript" src="../Order/js/dropzone-jquery.js"></script>
<script type="text/javascript" src="../Order/js/dropzone-dropzone.min.js"></script>
<script src="../Order/js/ckeditor-ckeditor.js"></script>
...
<br><br><br><br><script>
  ...
  <div class="form-group">
    <label for="task_id" class="col-sm-12">Type of Task</label>
    <select   class="form-control custom-select border-default col-sm-12" id="task_id" onchange="task(this)" required>
      <?php 
                  //query for courses
                  $sqi="select * from order_task where task_name='$task_id'";
                  ...
                  while ($row=mysqli_fetch_array($record,MYSQLI_BOTH))
                  {
                    $pat1=$row["task_name"];
                    $pat=$row["rate"];
                    echo "<option value=\"$pat\" class=\"text-center\" selected>$pat1</option>";
...
<?php 
  ...
  <input type="hidden" id='show' name="task_id" value="<?php echo $task_id;?>"/>
    ...
    function task(thisObj)
{
  ...
  document.getElementById('show').value = thisObj.options[thisObj.selectedIndex].text;
    ...
    <input type="text" name="topic" class="form-control col-sm-12 border-default" id="order_title" aria-describedby="emailHelp" value="<?php echo $topic; ?>" ></div>

    <div class="form-group">
    <label for="subject_id" class="col-sm-12">Subject Area</label>
   <select name="" class="form-control col-sm-12 custom-select border-default" id="subject_id" onClick="showSelected1(this)" required>
    <?php 
                  //query for courses
                  $sqi="select * from order_subject where subject_name='$subject_id'";
                  $record=mysqli_query($conn,$sqi);
                  
                  while ($row=mysqli_fetch_array($record,MYSQLI_BOTH))
                  {
                    $pat1=$row["subject_name"];
                    $pat=$row["rate"];
...
<?php 
  ...
  <input type="hidden" id='show1' name="subject_id" value="<?php echo $subject_id ;?>"/>
  <script type="text/javascript">
    function showSelected1(thisObj)
{

  document.getElementById('show1').value = thisObj.options[thisObj.selectedIndex].text;
...
<div class="input-group row col-sm-12">
          ...
          <input type="text" id="order_pages" name="order_pages" class="form-control input-number border-default col-sm-12 text-center" value="<?php echo $order_pages; ?>" min="0" max="100"><span class="input-group-btn">
...
<div class="row col-sm-12 wordcount">  275 word approx </div>
   ...
   <select name="" class="col-sm-12 form-control  custom-select border-default" id="deadline_id" onClick="showSelected(this)" required>
    <?php 
                  //query for courses
                  $sqi="select * from order_deadline where name='$deadline_id'";
                  $record=mysqli_query($conn,$sqi);
...
<?php 
 ...
 <input type="hidden" id='show2' name="deadline_id" value="<?php echo $deadline_id ;?>"/>
  <script type="text/javascript">
    function showSelected(thisObj)
{
  ...
  <div class="form-group">
    <label for="academic_level_id" class="col-sm-12">Academic level</label>
    <select name="" class="form-control custom-select border-default col-sm-12" id="academic_level_id" onClick="showSelected2(this)" required>
      <?php 
                  //query for courses
                  $sqi="select * from order_aclevel where academic_name='$academic_level_id'";
                  $record=mysqli_query($conn,$sqi);
                  
                  while ($row=mysqli_fetch_array($record,MYSQLI_BOTH))
                  {
                    $pat1=$row["academic_name"];
                    $pat=$row["rate"];
...
<input type="hidden" id='Academic' name="academic_level_id" value="<?php echo $academic_level_id ;?>"/>
  <script type="text/javascript">
    function showSelected2(thisObj)
{

  document.getElementById('Academic').value = thisObj.options[thisObj.selectedIndex].text;
...
$(function() {
    ...
    <label for="quality_id" class="col-sm-12">Quality Level</label>
    <select name="" class="form-control custom-select border-default col-sm-12" id="quality_id"  onClick="showSelected3(this)" required>
      <?php 
                  //query for courses
                  $sqi="select * from order_quality where quality_name='$quality_id'";
                  $record=mysqli_query($conn,$sqi);
                  
                  while ($row=mysqli_fetch_array($record,MYSQLI_BOTH))
                  {
                    $pat1=$row["quality_name"];
                    $pat=$row["rate"];
...
<input type="hidden" id='quality' name="quality_id"/>
  <script>
    function showSelected3(thisObj)
{

  document.getElementById('quality').value = thisObj.options[thisObj.selectedIndex].text;
...
<label class="col-sm-12">Sources</label>
  <div class="input-group col-sm-12">
          ...
          <input type="text" id="order_sources" name="order_sources" class="form-control border-default input-number col-sm-12 text-center" value="<?php echo $order_sources; ?>" min="0" max="100"><span class="input-group-btn">
...
<label class="col-sm-12">PowerPoint Slides</label>
  <div class="input-group col-sm-12">
          ...
          <input type="text" id="order_pptslides" name="order_pptslides" class="form-control border-default input-number col-sm-12 text-center" value="<?php echo $order_pptslides; ?>" min="0" max="50"><input type="hidden" id="config_priceperslide" name="config_priceperslide" value="6"><span class="input-group-btn">
   ...
   <div class="form-group">
      ...
      <div class="col-sm-12">
           ...
           <select name="order_citation" class="form-control custom-select border-default col-sm-12" id="exampleSelecte1"><option value="APA" >APA</option><option value="<?php echo $order_citation; ?>" selected>MLA</option><option value="Havard">Havard</option><option value="Other">Other</option></select><style>
    ...
    <textarea class="border-default" height="110px" name="order_instructions" id="editor" value="<?php echo $order_instructions; ?>"></textarea><script>
        ClassicEditor
...
<div class="col-sm-5 text-success font-weight-bold"><h4><div class="orderamountc">$</div></h4></div>
 <input id="order_amount" class="form-control fontbig form-control-plaintext" type="text" name="order_amount" value="$<?php echo $order_amount; ?>" /readonly>
  <input   type="hidden" name="order_amount1" value="$<?php echo $order_amount; ?>" /readonly>
...
<hr><div id="subcontact" class="form-group"><button  type="submit" name="orderedit"  class="btn col-sm-12 btn-warning" data-toggle="modal" data-target="#exampleModallogin" >UPDATE ORDER NOW</button></div>
...
totalp  = document.getElementById('payment-totalp');
 for (var i=0; i < addonp.length; i++) {
  ...
  $(document).on("change",function(){
    ...
    var cap=$('[id="academic_level_id"]').val();
    var tier= $('[id="quality_id"]').val();
    var cpn_value = $('[name="cpn_value"]').val();
    var spacing = 1;
    var subject = $('[id="subject_id"]').val();
    var tpaper = $('[id="task_id"]').val();
