...
if(!empty($_GET['item_number']) && !empty($_GET['tx']) && !empty($_GET['amt']) && !empty($_GET['cc']) && !empty($_GET['st'])){
	...
	if($prevPaymentResult->num_rows > 0){
		...
		//$payment_id = $conn->insert_id;
		 ...
		 mysqli_query($conn,"INSERT INTO payments(item_number,txn_id,payment_gross,currency_code,payment_status) VALUES('".$item_number."','".$txn_id."','".$payment_gross."','".$currency_code."','".$payment_status."')")or die(mysqli_error($conn));
...
$result_set=mysqli_query($conn,$query);
       ...
       //date_default_timezone_set('Africa/Nairobi');
        ...
        if($row->status>1){
             $total_pay=($payment_gross)+($row->$order_amount);
        require_once("include/connection.php");
		 mysqli_query($conn,"update order_file set paid_status='1',order_amount='$total_pay',status='2' where order_id='$item_number'")or die(mysqli_error($conn));
        }
        else{
              $query=mysqli_query($conn,"SELECT * FROM `order_file` WHERE `order_id` = '$item_number'")or die(mysqli_error($conn));
           $counter=mysqli_num_rows($query);
            if ($counter == 1){ 
              mysqli_query($conn,"update login_user set account_bal='$payment_gross' where user_id='$user_id'")or die(mysqli_error($conn));
            }
            else{
                  require_once("include/connection.php");
		 mysqli_query($conn,"update order_file set paid_status='1',order_amount='$payment_gross',status='2' where order_id='$item_number'")or die(mysqli_error($conn));
            }
