<?php
  ...
  $date = date("M-d-Y h:i A",strtotime("+0 HOURS"));
		...
		mysqli_query($conn,"insert into `chat` (chat_room_id, chat_msg, username,to_user, chat_date) values ('$id','$msg','writer','$to_user','$date')") or die(mysqli_error());
	}
?>
<?php
	if(isset($_POST['res'])){
	...
	?>
	<?php
		$query=mysqli_query($conn,"select * from `chat` where chat_room_id='$id'") or die(mysqli_error());
